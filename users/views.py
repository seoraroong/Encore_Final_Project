...
def get_user_from_db(email):
    try:
        print("************ get_user_from_db **********", email)
        user_data = user_collection.find_one({"email":email})
        # user_data = user_model_collection.find_one({"email":email})
        print("************ get_user_from_db **********", user_data)
        if user_data:
            return User(
        ...
        if form.is_valid():
            ...
            if user is not None:
                ...
                response_data = {
                    ...
                    # 'redirect': reverse('travel:service_main')  # 성공 시 리다이렉트할 URL
                }
                print("*********** login_view ********** 로그인 성공")
                return JsonResponse(response_data)
            else:
                # 로그인 실패
